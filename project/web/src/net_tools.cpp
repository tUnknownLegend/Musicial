...
bool operator==(const URL &lURL, const URL &rURL) {
...
Message getMessage() {
    ...
    for (uint64_t i = 0; i < message.playlistNumber; ++i) {
        ...
        std::cin >> str;
        auto p = static_cast<Platform>(std::stoi(str));
...
Message String2Message(const std::string &str) {
    ...
    for (uint64_t i = 0; i < result.playlistNumber * 2; ++i) {
        if (!(i % 2)) {
            auto p = static_cast<Platform>(std::stoi(blocks[i + 1 + 4]));
    ...
    for (uint64_t i = 0; i < result.songsNumber * 2; ++i) {
        if (!(i % 2)) {
            auto p = static_cast<Platform>(std::stoi(blocks[i + 1 + 5 + result.playlistNumber * 2]));
